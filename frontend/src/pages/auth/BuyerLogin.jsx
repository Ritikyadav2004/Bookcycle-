...
const BuyerLogin = () => {
  ...
  const onSubmit = async (data) => {
    try {
      const user = await authService.loginBuyer(data);
    ...
    <div className="w-full">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <Link to="/forgot-password" className="text-xs font-medium text-emerald hover:text-forest transition-colors">
              Forgot password?
            </Link>
          </div>
          <div className="relative">
        ...
        <button 
          ...
          className="w-full btn-primary py-3.5 flex items-center justify-center gap-2 group shadow-md"
        >
          {isSubmitting ? (
      ...
      <div className="mt-8 text-center text-sm">
        <p className="text-mutedText">
          ...
          <Link to="/buyer/register" className="font-semibold text-forest hover:text-emerald transition-colors inline-flex items-center gap-1">
      ...
      <div className="mt-12 pt-8 border-t border-forest/10">
        <p className="text-center text-xs text-mutedText mb-4">Are you a seller?</p>
        <Link to="/seller/login" className="w-full btn-ghost border border-forest/20 flex justify-center">
          Login as Seller
        </Link>
      </div>
    </div>
